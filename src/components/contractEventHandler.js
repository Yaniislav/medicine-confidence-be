...
const listen = async (event) => {
  ...
  if (patient && doctor) {
    if (event.name === eventNames.PAYMENT) {
      ...
      if (event.type === paymentEventTypes.userPaidForArrengement) {
      ...
      } else if (event.type === paymentEventTypes.arrangementCanceled) {
      ...
      notification.message = `Doctor ${doctor.firstName} ${doctor.lastName} tries to add entry to your history`;
      ...
      notification.message = `Doctor ${doctor.firstName} ${doctor.lastName} has added entry to your history`;
      notification.recipientAddress = patient.ethAddress;
      notification.sourceAddress = doctor.ethAddress;
      notification.senderId = doctor._id;
    } else return;
